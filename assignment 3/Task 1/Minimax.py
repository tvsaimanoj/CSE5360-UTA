...
class Minimax:
	...
	def makeDecision(self):
		...
		minimumValue = []
		posmoves = possibleMoves(self.game.gameBoard)
		#self.game.nodeDepth = 0
		#v = self.maxiValue(self.game,99999,-99999)
		#chosen = 0

		for move in posmoves:

			rslt = result(self.game,move)
			minimumValue.append( self.miniValue(rslt,99999,-99999) )
			# if v == self.utility(rslt):
			# 	chosen = move
			# 	break

		chosen = posmoves[minimumValue.index( max( minimumValue ) )]
		return chosen
